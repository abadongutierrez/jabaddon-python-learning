...
const playResponseAudio = (function () {
  ...
  return function Sound(src) {
    console.log("Playing audio with src length:", src ? src.length : 0);
    ...
    snd.addEventListener("error", function (e) {
      ...
      console.error("Audio error code:", snd.error ? snd.error.code : "unknown");
      console.error("Audio error message:", snd.error ? snd.error.message : "unknown");
    });
    ...
    if (playPromise !== undefined) {
      playPromise
        ...
        .catch((error) => {
          ...
          console.error("Audio src:", src.substring(0, 100) + "...");
        });
    }
...
const populateBotResponse = async (userMessage) => {
  ...
  $("#message-list").append(
    ...
    }</div><button id='${repeatButtonID}' class='btn volume repeat-button' onclick='playResponseAudio(&quot;data:audio/mpeg;base64,&quot; + responses[botRepeatButtonIDToIndexMap[this.id]].openaiResponseSpeech);console.log(this.id)'><i class='fa fa-volume-up'></i></button></div>`
  );

  playResponseAudio("data:audio/mpeg;base64," + response.openaiResponseSpeech);
